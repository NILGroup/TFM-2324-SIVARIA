...
def main():            
    ...
    for i in range(5): 
        with open('datasets/professional/dataset' + str(i + 1) + '.csv', 'w', newline='') as file:
            writer = csv.DictWriter(file, fieldnames=constants.PROFESSIONAL_FIELDS)
            writer.writeheader()
            data = []
            for csvIndex in range(25):
                ...
                singleData['Situacion economica familiar precaria'] = constants.YES_NO_ANSWERS[random.randint(0,len(constants.YES_NO_ANSWERS) - 1)]
                singleData['Estudios de la madre'] = constants.YES_NO_ANSWERS[random.randint(0,len(constants.YES_NO_ANSWERS) - 1)]
                singleData['Estudios del padre'] = constants.YES_NO_ANSWERS[random.randint(0,len(constants.YES_NO_ANSWERS) - 1)]
                singleData['Supervision parental insuficiente'] = constants.YES_NO_ANSWERS[random.randint(0,len(constants.YES_NO_ANSWERS) - 1)]
                singleData['Maltrato al adolescente'] = constants.YES_NO_ANSWERS[random.randint(0,len(constants.YES_NO_ANSWERS) - 1)]
                singleData['Duelo'] = constants.YES_NO_ANSWERS[random.randint(0,len(constants.YES_NO_ANSWERS) - 1)]
                singleData['Ingreso familiar mensual'] = random.randint(400, 4000)
